# Local reimplementation of synthdid's internal contract3 function.
# Computes the tensor contraction of a 3D array X with a vector v:
# result[i,j] = sum_k X[i,j,k] * v[k]
# When X is 2D, returns X * v (scalar multiplication).
...
mc_estimate <- function(Y, N0, T0) {
  if (!requireNamespace("MCPanel", quietly = TRUE)) {
    stop(
      "The 'MCPanel' package is required for the 'mc' estimator but is not installed.\n",
      "Install it with: devtools::install_github('susanathey/MCPanel')",
      call. = FALSE
    )
  }
  N1 <- nrow(Y) - N0
...
mc_placebo_se <- function(Y, N0, T0, replications = 200) {
  ...
  res <- sqrt((replications - 1) / replications) * stats::sd(replicate(replications, theta(sample(1:N0))))
  return(res)
}
