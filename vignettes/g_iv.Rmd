...
```{r ivreg-diagnostics, eval=FALSE}
# Full diagnostics
...
```{r sensemakr-contour, eval=FALSE, fig.width=7, fig.height=5}
...
```{r sensemakr-benchmark, eval=FALSE, fig.width=7, fig.height=5}
...
```{r lasso-iv, eval=FALSE, error = TRUE}
library(hdm)
...
```{r randomization-inference-iv, eval=FALSE}
# Simulate IV data
set.seed(42)
n <- 200
...
```{r mendelian-randomization, eval=FALSE}
...
```{r larf_viz, eval=FALSE, fig.width=9, fig.height=5}
...
```{r rotemberg_weights, eval=FALSE}
...
```{r rotemberg_concentration, eval=FALSE}
...
```{r bartik_leaveoneout, eval=FALSE}
...
```{r many_iv_simulation, eval=FALSE}
