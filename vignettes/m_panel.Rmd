...
```{r balanced-panel-demo, eval = requireNamespace("fixest", quietly = TRUE)}
library(fixest)
...
decompose_variance <- function(df, var, unit_var = "unit") {
  ...
  unit_means <- df |>
    ...
    summarise(unit_mean = mean(.data[[var]], na.rm = TRUE), .groups = "drop")
...
compute_icc <- function(df, outcome = "y", unit_var = "unit") {
  ...
  unit_means <- df |>
    group_by(across(all_of(unit_var))) |>
    summarise(u_mean = mean(.data[[outcome]]), n_t = n(), .groups = "drop")
...
```{r pooled-ols, eval = requireNamespace("fixest", quietly = TRUE)}
library(fixest)
...
```{r ols-se-comparison, eval = requireNamespace("fixest", quietly = TRUE)}
...
```{r twfe, eval = requireNamespace("fixest", quietly = TRUE)}
# Unit FE only
...
```{r twoway-cluster, eval = requireNamespace("fixest", quietly = TRUE)}
...
```{r extract-fe, eval = requireNamespace("fixest", quietly = TRUE)}
...
```{r tidy-fe, eval = requireNamespace("fixest", quietly = TRUE)}
...
```{r re-model, eval = requireNamespace("plm", quietly = TRUE)}
library(plm)
...
```{r hausman-test, eval = requireNamespace("plm", quietly = TRUE)}
fe_plm <- plm(y ~ x1 + x2,
              data   = panel_pdata,
              effect = "individual",
              model  = "within")
...
```{r mundlak-cre, eval = requireNamespace("plm", quietly = TRUE) && requireNamespace("car", quietly = TRUE)}
library(car)
# Compute unit means
panel_cre <- panel_df |>
  group_by(unit) |>
...
```{r fd-estimator, eval = requireNamespace("plm", quietly = TRUE)}
...
```{r fd-vs-fe, eval = requireNamespace("plm", quietly = TRUE)}
...
```{r fd-iv, eval = requireNamespace("plm", quietly = TRUE)}
...
```{r between-est, eval = requireNamespace("plm", quietly = TRUE)}
...
```{r adf-per-unit, eval = requireNamespace("tseries", quietly = TRUE)}
library(tseries)
...
```{r adf-histogram, eval = requireNamespace("tseries", quietly = TRUE), fig.width=6, fig.height=4}
...
```{r kpss-test, eval = requireNamespace("tseries", quietly = TRUE)}
...
```{r ips-test, eval = requireNamespace("tseries", quietly = TRUE)}
...
```{r wooldridge-test, eval = requireNamespace("plm", quietly = TRUE)}
library(plm)
...
```{r bg-test, eval = requireNamespace("plm", quietly = TRUE)}
...
```{r ar1-correction, eval = requireNamespace("plm", quietly = TRUE)}
...
```{r pesaran-cd, eval = requireNamespace("plm", quietly = TRUE)}
...
```{r frees-test, eval = requireNamespace("plm", quietly = TRUE)}
...
```{r hetero-test, eval = requireNamespace("plm", quietly = TRUE)}
...
```{r modified-wald, eval = requireNamespace("fixest", quietly = TRUE)}
...
unit_resid_var <- panel_with_resid |>
  group_by(unit) |>
  summarise(resid_var = var(fe_resid), .groups = "drop")
...
```{r fgls-hetero, eval = requireNamespace("plm", quietly = TRUE)}
# FGLS: weight observations by inverse estimated variance
unit_vars <- panel_with_resid |>
  group_by(unit) |>
  summarise(sigma2_i = var(fe_resid), .groups = "drop")
...
```{r nickell-bias-demo, eval = requireNamespace("fixest", quietly = TRUE)}
set.seed(1234)
N_dyn <- 200
T_dyn <- 6
...
```{r arellano-bond, eval = requireNamespace("plm", quietly = TRUE)}
library(plm)
...
```{r system-gmm, eval = requireNamespace("plm", quietly = TRUE)}
# System GMM
bb_model <- pgmm(
...
```{r sargan-test, eval = requireNamespace("plm", quietly = TRUE)}
...
```{r ab-ar-tests, eval = requireNamespace("plm", quietly = TRUE)}
...
```{r engle-granger-panel, eval = requireNamespace("tseries", quietly = TRUE)}
library(tseries)
...
```{r bai-factor, eval = requireNamespace("fixest", quietly = TRUE)}
set.seed(321)
N_f <- 50
T_f <- 15
...
```{r twfe-vs-ife, eval = requireNamespace("fixest", quietly = TRUE)}
...
```{r fe-logit, eval = requireNamespace("fixest", quietly = TRUE)}
set.seed(444)
...
```{r fe-poisson, eval = requireNamespace("fixest", quietly = TRUE)}
set.seed(555)
...
```{r count-comparison, eval = requireNamespace("fixest", quietly = TRUE)}
...
```{r distributed-lag, eval = requireNamespace("fixest", quietly = TRUE)}
# Create lags
panel_lags <- panel_df |>
  arrange(unit, year) |>
  group_by(unit) |>
...
```{r local-projections, eval = requireNamespace("fixest", quietly = TRUE)}
panel_lp <- panel_df |>
  arrange(unit, year) |>
  group_by(unit) |>
  mutate(
...
```{r lp-plot, eval = requireNamespace("fixest", quietly = TRUE), fig.width=6, fig.height=4}
...
vd_long <- vd_results |>
  select(variable, pct_between, pct_within) |>
...
```{r acf-panel-resid, eval = requireNamespace("fixest", quietly = TRUE), fig.width=7, fig.height=5}
...
```{r within-between-scatter, eval = requireNamespace("gridExtra", quietly = TRUE), fig.width=7, fig.height=4}
...
p_between <- wb_df |>
  group_by(unit) |>
  summarise(x1_mean = mean(x1), y_mean = mean(y), .groups = "drop") |>
...
```{r four-column-table, eval = requireNamespace("fixest", quietly = TRUE)}
...
```{r causal-table, eval = requireNamespace("fixest", quietly = TRUE) && requireNamespace("modelsummary", quietly = TRUE)}
causal_table(
  models      = list(
    "(1) Pooled OLS" = m1,
    "(2) Unit FE"    = m2,
...
```{r tidy-pipeline, eval = requireNamespace("fixest", quietly = TRUE)}
...
all_results <- bind_rows(
  ...
  select(model, term, estimate, std_error, p_value, ci_lower, ci_upper)
...
```{r coef-plot-estimators, eval = requireNamespace("fixest", quietly = TRUE), fig.width=7, fig.height=4}
...
```{r driscoll-kraay, eval = requireNamespace("fixest", quietly = TRUE)}
library(fixest)
...
```{r het-te-detect, eval = requireNamespace("fixest", quietly = TRUE)}
set.seed(2025)
...
```{r binscatter-het, eval = requireNamespace("fixest", quietly = TRUE), fig.width=6, fig.height=4}
...
```{r spec-curve-panel, eval = requireNamespace("fixest", quietly = TRUE), fig.width=7, fig.height=5}
...
```{r panel-power, eval = requireNamespace("fixest", quietly = TRUE), fig.width=6, fig.height=4}
set.seed(3333)
...
```{r firm-unit-root, eval = requireNamespace("tseries", quietly = TRUE)}
library(tseries)
...
```{r firm-models, eval = requireNamespace("fixest", quietly = TRUE) && requireNamespace("plm", quietly = TRUE)}
library(fixest)
library(plm)
...
```{r firm-diagnostics, eval = requireNamespace("plm", quietly = TRUE) && requireNamespace("fixest", quietly = TRUE)}
# Hausman test
...
```{r firm-final, eval = requireNamespace("fixest", quietly = TRUE)}
...
```{r firm-plot, eval = requireNamespace("fixest", quietly = TRUE), fig.width=6, fig.height=4}
