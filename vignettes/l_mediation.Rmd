...
description: >
  Causal mediation analysis in R with causalverse: decomposing a total effect into direct and indirect paths to identify the mechanism.
author: "Mike Nguyen"
date: "`r Sys.Date()`"
...
```{r mediation-path-diagram, echo=FALSE, fig.width=7, fig.height=3.5, fig.align='center'}
library(ggplot2)
...
nodes <- data.frame(
  ...
  x     = c(1, 3.5, 6),
  y     = c(2, 3.5, 2),
  fill  = c("#EFF3FF", "#FEE6CE", "#E5F5E0"),
  color = c("#2171B5", "#D94801", "#31A354")
)

ggplot() +
  # Arrows
  annotate("segment", x = 1.65, xend = 2.85, y = 2.35, yend = 3.15,
           arrow = arrow(length = unit(0.2, "cm"), type = "closed"),
           linewidth = 1, color = "#D94801") +
  annotate("segment", x = 4.15, xend = 5.35, y = 3.15, yend = 2.35,
           arrow = arrow(length = unit(0.2, "cm"), type = "closed"),
           linewidth = 1, color = "#31A354") +
  annotate("segment", x = 1.7, xend = 5.3, y = 1.75, yend = 1.75,
           arrow = arrow(length = unit(0.2, "cm"), type = "closed"),
           linewidth = 1, color = "#2171B5") +
  # Path labels
  annotate("text", x = 1.9, y = 3.05, label = "path a", fontface = "italic",
           size = 4, color = "#D94801") +
  annotate("text", x = 5.1, y = 3.05, label = "path b", fontface = "italic",
           size = 4, color = "#31A354") +
  annotate("text", x = 3.5, y = 1.45, label = "path c\u2032 (direct)", fontface = "italic",
           size = 4, color = "#2171B5") +
  # Node boxes
  geom_label(data = nodes, aes(x = x, y = y, label = label),
             fill = nodes$fill, color = nodes$color,
             size = 4.5, fontface = "bold", label.size = 1.2,
             label.padding = unit(0.5, "lines"), label.r = unit(0.2, "lines")) +
  coord_cartesian(xlim = c(0.2, 6.8), ylim = c(0.8, 4.2)) +
  theme_void() +
...
```{r path-diagram, echo=FALSE, fig.width=8, fig.height=4}
...
```{r sensitivity-viz, fig.width=7, fig.height=5, eval=FALSE}
