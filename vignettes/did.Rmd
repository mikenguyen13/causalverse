...
agg_random <- data %>%
  ...
  summarise(units_treated = sum(treatment_random))

agg_systematic <- data %>%
  group_by(time_period) %>%
  summarise(units_treated = sum(treatment_systematic))

# Plotting
plot_random <- ggplot(agg_random, aes(x = time_period, y = units_treated)) +
  geom_point() +
  labs(title = "Random Time Assignment")

plot_systematic <- ggplot(agg_systematic, aes(x = time_period, y = units_treated)) +
  geom_point() +
...
data <- data.frame(treatment = c(rep(0, 50), rep(1, 50)),
                   ...
                   t.test(characteristic ~ treatment, data = data)
```
...
```{r}
lm_result <- lm(characteristic ~ treatment, data = data)
summary(lm_result)
```

2.  **For Multiple Pre-treatment Characteristics:**

    -   **Multivariate Regression:** Run a regression of each pre-treatment characteristic on the treatment dummy. This allows for the simultaneous assessment of balance on multiple characteristics. You can include all characteristics in a single regression as dependent variables using seemingly unrelated regressions (SUR) if you suspect the error terms of the different regression equations are correlated.

```{r}
data$characteristic2 <- rnorm(100)

lm_multivariate <-
  lm(cbind(characteristic, characteristic2) ~ treatment, data = data)
summary(lm_multivariate)
```

All of the coefficients in each regression are not significant. Hence, we don't have any concerns. However, to be more rigorous, we should estimate all regressions simultaneously using SUR

```{r}
# install.packages("systemfit")
library(systemfit)

equation1 <- characteristic ~ treatment
equation2 <- characteristic2 ~ treatment
...
For characteristic: The treatment effect is not statistically significant (p-value = 0.27422), implying no evidence against random unit assignment based on this characteristic.

For characteristic2: The treatment effect is marginally significant (p-value = 0.092367). While not significant at the standard 0.05 level, it's noteworthy at the 0.10 level. This suggests weak evidence against random unit assignment based on this characteristic.

In conclusion, the results largely support the idea of random unit assignment, especially for characteristic. However, there's weak evidence against it for characteristic2

-   **Hotelling's T-squared test:** This is the multivariate counterpart of the T-test and is designed to test the mean vector of two groups. It's useful when you have multiple pre-treatment characteristics and you want to test if their mean vectors differ between the treated and control groups.
...
treated_data <-
  data[data$treatment == 1, c("characteristic", "characteristic2")]
control_data <-
  data[data$treatment == 0, c("characteristic", "characteristic2")]

hotelling_test_res <- hotelling.test(treated_data, control_data)
hotelling_test_res
```

-   **Matching:** This is a more advanced technique. For example, you model the probability of being treated based on pre-treatment characteristics using a logistic regression. Then, treated units are matched to control units based on this probability (the propensity score). After matching, you can test for balance in the pre-treatment characteristics between the treated and control groups. Ideally, post-matching differences should be small and insignificant. But this approach is more of a remedy for your non-randomness issue. Typically, if you have to employ this method, readers often become skeptical of your settings. Unless this is the robustness check, where you show that with and without matching, your results are consistent. But if your results are based on the foundation of matching, people can be doubtful.
...
m.out <- matchit(treatment ~ characteristic + characteristic2, data = data, method = "nearest")
matched_data <- match.data(m.out)

# After matching, you can test for balance using t-tests or regressions as mentioned above.
t.test(characteristic ~ treatment, data = matched_data)
...
ggplot(data, aes(x = characteristic, fill = factor(treatment))) +
...
for (var in variables) {
  p <- ggplot(data, aes(x = !!sym(var), fill = factor(treatment))) +
    geom_density(alpha = 0.5) +
    labs(fill = "Treatment") +
    ggtitle(paste("Density Distribution of", var, "by Treatment Group"))
