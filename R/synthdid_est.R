...
synthdid_est <-
  function(data,
    ...
    data <- data %>%
      dplyr::mutate(!!treat_stat_var := as.logical(!!rlang::sym(treat_stat_var))) %>%
      dplyr::select(dplyr::all_of(c(treat_stat_var, time_var, unit_id_var, treated_period_var, outcome_var)))
    ...
    # contract3: compute X %*% beta for 3D array X
