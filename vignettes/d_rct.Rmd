...
description: >
  Randomized Control Trials in R with causalverse: random assignment, power analysis, and estimating treatment effects from experimental data.
author: "Mike Nguyen"
date: "`r Sys.Date()`"
...
```{r balance-assessment, eval = TRUE}
...
The estimated treatment effect from the t-test is approximately `r round(-diff(t_result$estimate), 3)`. Note that `t.test()` computes "group 0 mean minus group 1 mean," so we negate the difference to get the treatment effect in the conventional direction.
...
```{r ri2-example, eval = requireNamespace("ri2", quietly = TRUE) && requireNamespace("randomizr", quietly = TRUE), error = TRUE}
library(ri2)
library(randomizr)
...
```{r grf-example, eval = requireNamespace("grf", quietly = TRUE)}
library(grf)
...
This tells us we need approximately `r ceiling(power_result$n)` observations per group (i.e., `r 2 * ceiling(power_result$n)` total) to detect an effect of 2.5 with 80% power.
...
```{r sensemakr-example, eval = requireNamespace("sensemakr", quietly = TRUE)}
library(sensemakr)
...
```{r grf-data, eval = requireNamespace("grf", quietly = TRUE)}
set.seed(42)
n <- 2000
p <- 6
...
```{r grf-fit, eval = requireNamespace("grf", quietly = TRUE)}
library(grf)
...
```{r grf-varimp, eval = requireNamespace("grf", quietly = TRUE)}
...
```{r grf-blp-cal, eval = requireNamespace("grf", quietly = TRUE)}
...
```{r grf-priority, eval = requireNamespace("grf", quietly = TRUE)}
...
```{r dml-plr, eval = requireNamespace("DoubleML", quietly = TRUE)}
library(DoubleML)
library(mlr3)
...
```{r dml-irm, eval = requireNamespace("DoubleML", quietly = TRUE)}
set.seed(42)
n <- 1000
...
```{r bart-fit, eval = requireNamespace("bartCause", quietly = TRUE)}
library(bartCause)
...
```{r bart-att, eval = requireNamespace("bartCause", quietly = TRUE)}
fit_att <- bartc(
  response  = y,
  treatment = treatment,
  confounders = data.frame(x1, x2, x3),
...
```{r bart-posterior, eval = requireNamespace("bartCause", quietly = TRUE)}
...
```{r policytree-scores, eval = requireNamespace("policytree", quietly = TRUE) && requireNamespace("grf", quietly = TRUE)}
library(policytree)
library(grf)
...
```{r policytree-fit, eval = requireNamespace("policytree", quietly = TRUE) && requireNamespace("grf", quietly = TRUE)}
...
```{r policytree-eval, eval = requireNamespace("policytree", quietly = TRUE) && requireNamespace("grf", quietly = TRUE)}
...
```{r bandits, eval = requireNamespace("banditsCI", quietly = TRUE)}
library(banditsCI)
...
```{r rar-plot, eval = TRUE, fig.width = 9, fig.height = 5}
...
```{r obf-spending, eval = TRUE, fig.width = 9, fig.height = 5}
...
```{r bayes-adaptive, eval = TRUE, fig.width = 9, fig.height = 5}
set.seed(42)
...
```{r consort-diagram, eval = TRUE, echo = FALSE, fig.width = 10, fig.height = 8}
...
```{r subgroup-forest, eval = TRUE, fig.width = 9, fig.height = 6}
set.seed(42)
n <- 800
...
```{r dose-response, eval = TRUE, fig.width = 9, fig.height = 5}
set.seed(42)
n <- 500
...
```{r compliance-plot, eval = TRUE, fig.width = 8, fig.height = 6}
set.seed(42)
n <- 600
...
```{r power-curve-effect-sizes, eval = TRUE, fig.width = 9, fig.height = 6}
...
```{r late-comparison-plot, eval = TRUE, fig.width = 9, fig.height = 5}
